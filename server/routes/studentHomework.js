...
// POST /api/student-homework/:id/verify-grade - Verify grade with screenshot
router.post('/:id/verify-grade', checkJwt, extractUser, requireStudent, upload.single('screenshot'), async (req, res) => {
  try {
    const auth0Id = req.userInfo.auth0_id;
    const user = await User.findOne({ auth0_id: auth0Id });
    if (!user) {
    ...
    const tolerance = 2;
    const isMatch = Math.abs(extractionResult.grade - homework.claimed_grade) <= tolerance;

    // Update homework with verification data
    homework.grade_verification_status = isMatch ? 'verified' : 'pending_review';
    homework.extracted_grade_data = {
      extracted_grade: extractionResult.grade,
      extracted_total: extractionResult.total,
      confidence: extractionResult.confidence,
      raw_text: extractionResult.rawText
    };
    homework.grade_screenshot_path = req.file.path;

    if (isMatch) {
      homework.verified_grade = extractionResult.grade;
      homework.grade_verified_by = user._id;
      homework.grade_verified_at = new Date();
    }
    ...
    res.json({
      success: true,
      claimedGrade: homework.claimed_grade,
      ...
      validation,
      rawText: extractionResult.rawText,
      verificationStatus: homework.grade_verification_status,
      message: isMatch ? 'Grade verification successful!' : 'Grade mismatch - pending lecturer review'
    });
